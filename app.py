...
def driver():
    ...
    target_date_str = request.args.get('target_date') # 💡 추가: 날짜 파라미터 받기
    dispatches = []
    route_chunks = [] 
    date_str = ""
    active_notices = []
    target_date = datetime.now().date() # 기본값 세팅
    ...
    # 💡 [추가] 마지막에 target_date를 html(프론트엔드)로 함께 넘겨줍니다.
